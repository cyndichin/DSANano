...
# <span class="graffiti-highlight graffiti-id_ldbkw0c-id_xf9p4ln"><i></i><button>Hide Solution</button></span>
...
class Queue:
    ...
    def _handle_queue_capacity_full(self):
        ...
        index = 0 
        for item in range(self.front_index, len(old_arr)):
            self.arr[index] = old_arr[item]
            index += 1
        
        for item in range(0, self.front_index):
            self.arr[index] = old_arr[item]
            index += 1
