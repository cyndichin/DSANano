# ========================================================
# COLLECTIONS
# Why data structures? "extensions of collections"
...
# The computer then finds a block of memory and sets aside the space for the array.
# Importantly, the space that gets set aside is one, continuous block. 
# That is, all of the elements of the array are contiguous, meaning that they are all next to one another in memory.

# Another key characteristic of an array is that
...
# For example, if the first element in the array is at memory location 00 and the elements are 24 bytes, then the next element would be at location 00 + 24 = 24. 
# And the one after that would be at 24 + 24 = 48, and so on.
...
# For example, later in this lesson we'll look at linked lists, where each list item points to the next list item—but where the items themselves may be scattered in different locations of memory. 
# In this case, knowing the location of the first item in the list does not mean you can simply calculate the location of the other items. 
# This means we cannot use indices to directly access the list items as we would in an array. We'll explore linked lists in more detail shortly.
